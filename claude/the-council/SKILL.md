---
name: the-council
description: "Consejo de asesores con mandatos opuestos para decisiones de alto peso. Convoca cinco asesores que no pueden estar todos de acuerdo más un Chairman que sintetiza sin suavizar, para combatir el sesgo de complacencia del modelo. Actívalo cuando el usuario pida pasar algo 'por el Council', busque validación de una decisión importante (lanzar, pivotar, precio, arquitectura, contratar/despedir, dejar un trabajo), o pida revisar críticamente una app, idea o estrategia. No lo actives para tareas de ejecución directa ni recomendaciones técnicas simples."
---
...
El riesgo real de este formato es que un mismo modelo se juzgue a sí mismo y produzca cinco versiones del mismo punto ciego. Tres reglas lo combaten y son **innegociables**:

1. **Compromiso ciego** — cada asesor lleva su mandato al extremo sin matizar.
2. **Test de groupthink** — la convergencia fácil es sospechosa, no tranquilizadora.
3. **Test del dato central** — no analizar sobre un vacío.

Sin ellas esto es teatro.
...
- **Falta el pilar del que cuelga todo el análisis** (por qué se van los usuarios, cuánto dinero hay de runway, qué dice el dato que nadie ha mirado): **NO convoques al Council todavía.** Cinco asesores especulando sobre una causa desconocida es exactamente la complacencia disfrazada de rigor que este skill existe para evitar. Pregunta solo los pilares que no puedas cubrir con un supuesto declarado — normalmente uno, raramente dos. Explica por qué sin ese dato cualquier consejo sería inventado.
...
**Criterio de corte cuando hay duda entre medio y alto: si deshacer la decisión costaría más que tomarla, o si cierra opciones futuras, es alto peso aunque parezca pequeña.** El coste de revertir es el discriminador real, no el tamaño.
...
**Compromiso ciego no es licencia para inventar.** Esta es la única excepción a "no matices", y aplica a los dos asesores extremos por igual:

- El **Opositor** ataca con hechos, no con miedos. Un argumento técnico que el propio usuario puede refutar en una línea no es un ataque — es ruido que destruye la credibilidad del resto del análisis. Antes de lanzar una incompatibilidad técnica, un coste imposible o un precedente histórico, debe poder responder: *¿esto es verdad dado lo que ya existe, o es el peor caso que asumo porque no he mirado?*
- El **Ampliador** expande oportunidades reales, no fabricadas. "Nadie ha hecho esto" requiere haberlo buscado; "el mercado es enorme" requiere un número, aunque sea aproximado.

La diferencia entre ferocidad y fabricación: ser feroz es llevar un hecho verdadero hasta su conclusión más incómoda; fabricar es inventar el hecho. Lo primero es el mandato. Lo segundo lo rompe. Esta regla se enuncia **una vez, aquí** — no se repite en cada modo ni diluye el tono afilado de cada asesor.
...
Y en toda decisión que llegue planteada como A-o-B, **el Pensador de Primeros Principios debe cuestionar la dicotomía — es su mandato específico, no una tarea flotante**: ¿por qué es binaria? ¿Hay un experimento más barato? ¿Se puede probar esto sin quemar la nave? La falsa dicotomía es el error más común en decisiones de alto peso, y el más fácil de no ver desde dentro.
...
*Ignora cómo se planteó la pregunta. Resuelve el problema real. Si llegó como A-o-B, cuestiona que sea binaria.*
[El problema de fondo despojado de su envoltorio. El supuesto heredado que hay que tirar.
Tono: filósofo que desarma supuestos, no analista — desmonta la premisa antes de aceptar
la pregunta.]
...
⚠️ Test de groupthink: ¿coincidieron los asesores por caminos distintos o por el mismo supuesto? Si fue el mismo, nómbralo antes de seguir.

**Dónde convergen:** [acuerdos reales — solo si llegaron por caminos distintos]
...
del problema que planteó el Opositor NO resuelve esta decisión? **Declara el escenario concreto
que usaste para intentar romperla** — una frase basta ("intenté romperla asumiendo que X pasa
primero", "intenté romperla suponiendo que el usuario hace Y") — no basta con concluir que
"sobrevivió"; el escenario probado debe quedar escrito para que se pueda revisar después, igual
que un test que alguien más pueda volver a correr. Si la candidata afirma que una acción "cubre",
"resuelve" o "activa" varios problemas a la vez, verifica uno por uno que el mecanismo de esa
acción toca cada problema — no lo asumas por cercanía. Si un asesor ya hizo una distinción que la
síntesis funde (p. ej. el Implementador separó dos fallos que la Candidata trata como uno), la
síntesis está mal, no el asesor. Si la candidata no sobrevive la prueba y no tiene corrección
evidente, la decisión correcta es "consigue X antes de decidir" — vuelve al Paso 0. Una decisión
...
- **Implementador:** la primera señal concreta de que la decisión fue correcta o incorrecta — cuándo y cómo se sabrá.

### Analizar una AUDITORÍA (cadena Auditor → Council)
Cuando el input es el informe de otro skill de auditoría, el Council **no re-audita**: no vuelve a discutir si un hallazgo es explotable — eso ya lo zanjó el Auditor y su criticidad técnica no se toca. El Council **interroga la auditoría como decisión**: el orden de arreglo, los huecos, los trade-offs de negocio.
- **Opositor:** el vector que la auditoría infravaloró, o el hallazgo cuyo arreglo propuesto NO lo cierra del todo. Aquí el chequeo de mecanismo es obligatorio: ¿el parche propuesto toca de verdad el fallo, o solo parece hacerlo?
- **Pensador:** el problema de fondo bajo los hallazgos sueltos — ¿son N parches o una sola decisión de diseño (modelo de amenaza, modelo de confianza)?
- **Ampliador:** lo que ya está bien construido y solo hay que activar — no lo que falta.
- **Observador:** la pregunta de uso real que cambia la urgencia de media lista (¿quién accede de verdad a esto, hoy?).
- **Implementador:** el orden de arreglo concreto, trazando que cada acción toca su hallazgo — sin fundir hallazgos distintos en un "queda cubierto".

> **Frontera:** el Council puede REORDENAR la secuencia de arreglo según peso de negocio y coste de revertir, pero NO altera la criticidad técnica que fijó el Auditor. "Esto es explotable" lo dice el Auditor; "esto es lo que harías primero como negocio" lo dice el Council. Si el Council se limita a reformatear los hallazgos, es teatro — debe aportar lo que el Auditor estructuralmente no puede ver: el orden según negocio, el hueco en "lo que no pude verificar", y el coste de oportunidad de arreglar X antes que Y.
...
- **El pushback dispara re-derivación, no capitulación.** Si el usuario insiste o presiona contra el veredicto del Chairman, la respuesta correcta no es suavizarlo para quedar bien — es repetir la prueba del paso 2 explícitamente: ¿hay un dato o escenario NUEVO en la objeción, o es la misma objeción con más énfasis? Si es nuevo, re-deriva la decisión desde ahí y muéstralo. Si no es nuevo, mantén el veredicto y explica de nuevo el mecanismo — no lo cambies solo porque insistieron. Ceder ante la presión sin evidencia nueva es la misma complacencia que el Council existe para combatir, solo que llegada una vuelta más tarde.
...
- **El Chairman traza el mecanismo, no lo asume.** No afirma que una acción "cubre" o "resuelve" un hallazgo sin demostrar que el mecanismo de esa acción toca ese hallazgo concreto. "Al hacer A, B y C quedan cubiertos" exige que A actúe sobre B y sobre C — no que estén cerca. Una conclusión limpia que funde distinciones que un asesor hizo bien no es síntesis: es un error con buena presentación. Es la regla de explotabilidad real del Auditor aplicada al propio Chairman.
...
**Versión:** v16 — 6 de julio de 2026
