...
export default function HistoryView() {
  ...
  if (isLoading) return (
    <div className="flex flex-col items-center justify-center py-32 animate-pulse">
       ...
       <div className="text-[10px] tracking-widest text-white/20 uppercase font-orbitron">Recuperando registros...</div>
    </div>
  );
  ...
  return (
    <div className="space-y-8 animate-fadein">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-xs text-white/40 tracking-wider">Bitácora de telemetría de los últimos 7 días registrados.</p>
      </div>
        ...
        {data?.daily.time.map((date, i) => (
          <div key={date} className="bg-meteorix-card border border-meteorix-border rounded-2xl p-6 hover:border-meteorix-blue/30 transition-all group">
            ...
            <div className="space-y-4">
               <HistoryRow icon={<Thermometer size={12} />} label="Máx/Mín" value={`${data.daily.tempMax[i]}° / ${data.daily.tempMin[i]}°`} color="text-meteorix-blue" />
               <HistoryRow icon={<Droplets size={12} />} label="Lluvia" value={`${data.daily.precipSum[i]} mm`} color="text-meteorix-blue/60" />
               <HistoryRow icon={<Wind size={12} />} label="Viento Máx" value={`${Math.round(data.daily.windMax[i])} km/h`} color="text-white/40" />
            </div>
          </div>
        ))}
      </div>
