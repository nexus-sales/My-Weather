...
export default function SpaceWeatherWidget({ kpIndex, flareClass, auroraProbability, dataQuality, source }: SpaceWeatherWidgetProps) {
  ...
  let status = hasData ? t('space.quiet') : t('space.noData');
  let color = '#34d399'; // emerald-400
  let gpsRisk = t('space.none');

  if (hasData && kp > 4) {
    status = t('space.minorStorm');
    color = '#fbbf24';
    gpsRisk = t('space.low');
  }
  if (hasData && kp > 5) {
    status = t('space.solarStorm');
    color = '#f87171';
    gpsRisk = t('space.possible');
  }
  ...
  let auroraLabel = t('space.calculating');
  let auroraColor = '#71717a';
  if (auroraProbability !== null) {
    if (auroraProbability < 3) { auroraLabel = t('space.negligible'); auroraColor = '#71717a'; }
  ...
  return (
    <WidgetWrapper title={t('space.title')} icon={<Satellite size={14} style={{ color }} />} dataQuality={dataQuality} source={source}>
      <div className="w-full h-full flex flex-col justify-between p-2">
        <div className="flex justify-between items-start">
           <div className="flex flex-col">
              <span className="text-[10px] font-outfit text-white/60 uppercase tracking-widest">{t('space.geomagnetic')}</span>
        ...
        <div className="flex flex-col gap-3 my-4">
           ...
           <div className="flex items-center justify-between bg-zinc-800/40 px-3 py-2 rounded-lg border border-white/5">
             <div className="flex items-center gap-2 text-white/80">
               ...
               <span className="text-xs font-inter">{t('space.gpsRisk')}</span>
             </div>
           ...
           <div className="flex items-center justify-between bg-zinc-800/40 px-3 py-2 rounded-lg border border-white/5">
             <div className="flex items-center gap-2 text-white/80">
               ...
               <span className="text-xs font-inter">{t('space.aurora')}</span>
             </div>
        ...
        <div className="flex justify-between items-center border-t border-white/5 pt-2 mt-auto">
           <div className="flex flex-col">
              <span className="text-[9px] font-outfit text-white/50 uppercase tracking-widest">{t('space.kpIndex')}</span>
              <span className="text-xs text-white/80 font-medium">
                {hasData ? t('space.level', { kp: kp.toFixed(1) }) : t('space.unavailable')}
              </span>
           </div>
